...
**Live URL:** https://secure-gate-sandy.vercel.app
...
The most hardest part of building SecureGate was handling the authentication and verification flow across multiple layers at the same time. The surprising part was how many subtle security and user-experience tradeoffs were required even for a small authentication app. I expected the core flows to be fairly straightforward, but I quickly learned that protecting users properly required careful handling of expired tokens, consistent error messaging, rate limiting, and secure redirection. That made the project feel much more like designing a secure system than just wiring up forms and routes.
...
The biggest technical debt in SecureGate is duplicated request validation and error handling across the auth API routes. The signup, forgot-password, and reset-password handlers each parse request bodies, validate input with Zod, and return structured error responses in similar but independent ways. I would centralize this behavior in reusable helpers so validation remains consistent, security checks are simpler to audit, and future auth endpoints can be added without repeating the same boilerplate.

Refactored version:

```ts
async function validateRequest<T extends z.ZodTypeAny>(req: Request, schema: T) {
...
```
This refactor reduces duplicated code, improves maintainability, and keeps auth validation behavior consistent across the app.

## Part 5 — How This Changes How I Build
The experience reinforced that secure auth systems should favor shared, reusable validation and error handling instead of repeating the same guard logic across endpoints. Going forward, I will prioritize small, composable helpers for request validation and API responses so future features remain easier to audit, test, and extend without introducing new security debt.
